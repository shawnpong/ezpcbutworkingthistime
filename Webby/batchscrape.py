...
async def main():
    ...
    for div_element in soup.find_all("div", class_="column col-10 col-lg-8 col-sm-12"):
        for link in div_element.find_all("a"):
            ...
            if (
                link_count >= 100
            ):  # Check if the desired number of links have been processed
    ...
    async with httpx.AsyncClient() as batch_client:
        response = await batch_client.post(
            api_url + "manufacturers/batch_create/", json=manufacturers, headers=headers
        )
        print(response.content)
        if response.status_code == 200:
    ...
    async with httpx.AsyncClient() as batch_client:
        response = await batch_client.post(
            api_url + "sizes/batch_create/", json=sizes, headers=headers
        )
        print(response.content)
        if response.status_code == 200:
